...
#### 케이스 스터디: 디지털 트윈 PC가 MC 프로토콜 슬롯 점유 (2026-05-20)

**증상**: `192.168.3.200` PC에서 `A_Process_pendant.py` 실행 → `✅ PLC 연결` 출력은 정상 → 첫 QR 스캔 후 `❌ PLC 전송 실패: timed out`. 재실행해도 동일.

**진단 과정**: GX Works (PLC 150 프로젝트) → 진단 → Ethernet 진단 → **커넥션별 상태** 탭 확인.

![점유 상태](images/ethernet_diag_before_slot_occupied_by_97.png)

행 13: 포트 2000 / **교신 상대 IP `192.168.3.97`** / 프로토콜 **MC 프로토콜** / TCP 상태 **접속 중**.
즉 PLC 150의 MC 프로토콜 슬롯을 **다른 PC(.97)** 가 잡고 있었음. `.200` PC의 Python은 TCP 자체는 붙지만 PLC가 응답을 안 해서 `timed out`이 났던 것. 

→ 사내 확인 결과 `.97`은 **디지털 트윈 시뮬레이션을 돌리던 PC**였고, 거기서 동일한 Open Setting 슬롯을 잡고 MC 프로토콜로 폴링 중이었음.

**해결**:
1. 디지털 트윈 PC(.97)의 통신 세션 종료
2. 그러자 `.200` PC가 같은 슬롯을 인계받음

![해결 후](images/ethernet_diag_after_resolved_200.png)

행 13: **교신 상대 IP `192.168.3.200`** / MC 프로토콜 / 접속 중 — 자기 PC로 슬롯 잡힌 것 확인. (최신 에러 코드 4183은 이전 실패 시도의 잔재 — "최신 에러 코드 클리어" 버튼으로 지우면 깔끔.)

**근본 원인**: 해당 Ethernet Open Setting이 **동시 접속 1개**만 허용하는 구성이어서, 먼저 잡은 PC가 들고 있는 동안 다른 PC는 응답을 못 받음. 진단 화면의 **교신 상대 IP 컬럼이 본인 PC IP인지** 가 핵심 체크 포인트.

**재발 방지**:
- 같은 PLC를 두 PC에서 동시에 쓰려면 Open Setting 슬롯을 2개 이상 만들거나 IP 제한을 풀어야 함
- 평상 운영에서는 어느 PC가 MC 프로토콜을 잡고 있는지 사전에 합의
