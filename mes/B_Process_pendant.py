...
═══════════════════════════════════════════════════════════════════════
  ...
  [이 PC의 역할]
    - B150 트리거 감지 → 비전 검사 자동 실행 (YOLO)
    - 양품/불량 판정 → 공정 B PLC로 결과 통보 (M250/M260)
    - tbl_robot_b에 검사 결과 INSERT
    - M1140 종료 신호 → 로그 출력 (종합 처리는 공정 C가 담당)
  ...
  [연결 구성]
    ...
    모델   : C_VISION.pt (프로젝트 루트, 6 클래스: r/g/b_normal/crack)
             ※ 모델은 A/B/C 공통 사용. 공정 B 전용 모델이 있다면 교체.
...
PLC_MONITOR_IP = "192.168.3.160"   # 관제 PLC (M1130 종료 신호 폴링)
...
# ───── YOLO 모델 ─────
# 모델 파일은 프로젝트 루트(MES의 상위)에 있는 C_VISION.pt (A/B/C 공통)
MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'C_VISION.pt')
# 임계값 필터링 없음 — ultralytics YOLO 기본값(conf=0.25) 그대로 사용.
...
# 비전 검출 폴링 상태
# B150 ON 감지 → vision_active=True 진입 → 매 프레임 추론 시도 → 검출되면 처리
# B150 OFF (0 키 또는 PLC) 시 자동 해제. 타임아웃 없음 (검출될 때까지 또는 OFF까지).
...
last_vision_result = None      # 최근 비전 검사 결과 (M1130 종료 시 INSERT 용)
...
def record_robot_b_result(product_sn, tray_sn, vision_result, defect_type=None):
    """비전 검사 결과 → tbl_robot_b INSERT"""
    try:
        conn = pymysql.connect(**DB_CONFIG)
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO tbl_robot_b "
...
def execute_vision_inspection(frame):
    """비전 검사 실행 → PLC 양품/불량 통보 → tbl_robot_b INSERT"""
    print(f"  🔍 비전 검사 시작")
    ...
    if product_sn:
        ...
        if record_robot_b_result(product_sn, tray_sn, vision_result, defect_type):
            print(f"  ✅ tbl_robot_b INSERT: {product_sn} → {vision_result}")
    else:
...
def monitor_plc_signals():
    """B150(비전 트리거) + M1140(공정 B 종료) 폴링.

    B150 상승 엣지: 검출 폴링 모드 진입 (실제 추론은 vision_polling_step(frame)이 매 프레임)
    B150 하강 엣지: 검출 폴링 모드 중단 (외부에서 OFF 시킨 경우)
    M1140 상승 엣지: 공정 B 종료 로그 출력 (종합 처리는 C 공정에서 담당)
    """
    global prev_vision_trigger, prev_done_b, vision_active, last_annotated_frame, last_vision_result

    if plc is None:
        return

    curr_vision_trigger = read_plc_bit(ADDR_VISION_TRIGGER)
    curr_done_b = read_done_bit()   # 종료 비트는 관제 PLC 160에서 읽음

    # ───── B150 상승 엣지 → 검출 폴링 모드 시작 ─────
    if curr_vision_trigger and not prev_vision_trigger:
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 🔍 {ADDR_VISION_TRIGGER} ON 감지 — 검출 폴링 시작")
        vision_active = True
        last_vision_result = None   # 새 사이클 — 이전 비전 결과 초기화

    # ───── B150 하강 엣지 → 검출 폴링 중단 (외부에서 OFF) ─────
    if not curr_vision_trigger and prev_vision_trigger and vision_active:
        print(f"  ⏹ {ADDR_VISION_TRIGGER} OFF — 검출 폴링 중단")
        vision_active = False
...
def vision_polling_step(frame):
    """vision_active 모드일 때 매 프레임마다 호출 — 검출 시도 + 결과 처리.

    동작:
      - 미검출 → 다음 프레임 시도 (조용히)
      - 검출 성공 + 유효 클래스 → PLC 통보 + tbl_robot_b INSERT + 모드 해제
      - 알 수 없는 클래스 검출 → 계속 시도 (다음 프레임 기대)

    B150 OFF는 자동으로 안 함. PLC 래더가 처리하거나 사용자가 0 키로 직접 OFF.
    OFF 안 시키면 다음 1 키 누름에서 상승 엣지가 안 생겨 재트리거 불가.

    컨베이어 시간 무관 — B150 ON 동안 트레이가 카메라 앞에 도착하면 즉시 처리.
    """
    global vision_active, last_diag_at, last_annotated_frame, last_vision_result

    if not vision_active:
        return

    # 한 프레임 추론 시도 (검출될 때까지 또는 B150 OFF까지 반복)
    result = vision_inspect(frame)
    # YOLO가 박스 그린 프레임을 저장 → 메인 cv2 창에 실시간 표시용
    last_annotated_frame = result.get('annotated_frame')

    # ── 진단 로그 — 1초마다 1번 추론 상태 출력 ──
    now = time.time()
    if now - last_diag_at >= 1.0:
        cls = result.get('class_name') or '(객체 없음)'
        conf = result.get('confidence', 0.0)
        detected = result.get('detected', False)
        status = "✓ 검출" if detected else "× 미검출"
        print(f"  [추론] {status}: class={cls}, conf={conf:.2f}")
        last_diag_at = now

    if not result['detected']:
        return   # 미검출 — 다음 프레임 시도

    if result['is_normal'] is None:
        # 인식 안 되는 클래스 (양품/불량 매핑 외) — 다음 프레임에서 다시
        return

    # ───── 검출 성공 → 결과 처리 ─────
    judgment = "양품 ✅" if result['is_normal'] else "불량 ❌"
    print(f"  검출: {result['class_name']} (신뢰도: {result['confidence']:.2f}) → {judgment}")

    # 1. PLC 양품/불량 통보
    send_vision_result_to_plc(result['is_normal'])

    # 2. 비전 결과를 메모리에 저장 — 실제 DB INSERT는 M1130 (공정 완료) 시점에 수행
    vision_result = 'OK' if result['is_normal'] else 'NG'
    defect_type = None if result['is_normal'] else result['class_name']
    last_vision_result = {
    ...
    print(f"  💾 비전 결과 메모리 저장 — M1130(공정 완료) 시 DB INSERT 예정 (현재: {vision_result})")

    # 검출 완료 → 모드만 해제 (B150 OFF는 PLC 래더 또는 사용자가 0 키로 처리)
    vision_active = False
    last_annotated_frame = None   # 검출 완료 후엔 원본 프레임으로 복귀
...
cv2.namedWindow('Process B - Vision Inspection (Pendant Mode)', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Process B - Vision Inspection (Pendant Mode)', 960, 720)
...
print(f"      └─ 객체 검출 시 → PLC 양품/불량 통보 + DB 기록")
print(f"      └─ {ADDR_VISION_TRIGGER} OFF는 PLC/사용자가 처리 (다음 트리거 위해 필요)")
print(f"    {ADDR_DONE_B} 감지 → 공정 B 종료 로그 (종합 처리는 공정 C)")
print("")
print("  [수동 제어]")
...
print(f"    {ADDR_NORMAL}      : 양품 통보 (Vision → PLC)")
print(f"    {ADDR_CRACK}     : 불량 통보 (Vision → PLC)")
print(f"    {ADDR_DONE_B}  : 공정 B 종료 (PLC 160 → Vision)")
print("")
print("  [로봇 제어 방식]")
print("    CC-Link → PLC 자체 센서/래더 → 펜던트 IF (공정 A/C와 동일)")
...
while True:
    ...
    # PLC 신호 폴링 (B150 / M1140 상승 엣지 감지 → 검출 모드 진입)
    monitor_plc_signals()
    ...
    cv2.imshow('Process B - Vision Inspection (Pendant Mode)', display_frame)
    ...
    if key == ord('q'):
        ...
        # B150 ON — PLC 140 시작 트리거 흉내 (공정 A 우회 디버그)
        # monitor_plc_signals이 다음 폴링에서 상승 엣지 감지 → 검출 폴링 자동 시작
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 🟢 [수동] {ADDR_VISION_TRIGGER} ON")
        set_b150(1)
    elif key == ord('0'):
        # B150 OFF — 재트리거 준비 (다음 1 키로 새 상승 엣지 만들기)
